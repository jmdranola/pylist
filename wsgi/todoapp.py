...
@app.route('/')
def todolist():
	con = sql.connect(os.path.join(os.environ.get('OPENSHIFT_DATA_DIR'), 'C:\python-todo\pylist\magedb.db'))
...
def add():
	if request.method == 'POST':
		try:
			...
			try:
				with sql.connect(os.path.join(os.environ.get('OPENSHIFT_DATA_DIR'), 'C:\python-todo\pylist\magedb.db')) as con:
					cur = con.cursor()
