...
conn = sqlite3.connect(os.path.join(os.environ.get('OPENSHIFT_DATA_DIR'), 'wsgi/magedb.db'))
...
def add():
	if request.method == 'POST':
		try:
			con = sql.connect(os.path.join(os.environ.get('OPENSHIFT_DATA_DIR'), 'wsgi/magedb.db'))
			con.row_factory = sql.Row
			...
			try:
				with sql.connect(os.path.join(os.environ.get('OPENSHIFT_DATA_DIR'), 'wsgi/magedb.db')) as con:
					cur = con.cursor()
